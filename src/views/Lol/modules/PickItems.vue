<template>
  <div class="flex w-[600px]">
    <div class="flex-1 h-[216px] border-r border-gray-700 relative" :class="{
      ...
      'blink-animation': getIsInProgress(index),
      ...
      <div class="absolute pl-2 bottom-2">
        {{ formatMemberName(index - 1) }} {{ teamMembers?.[index - 1]?.championId }}
      </div>
    </div>
  </div>
</template>
...
const pickActions = computed(() => isRedTeam ? selectStore.pickActions.red : selectStore.pickActions.blue);
const inProgressPlayers = computed(() => pickActions.value.reduce((pre, cur) => {
  if (cur.isInProgress) {
    const playerIndex = isRedTeam ? (cur.actorCellId - 5) : cur.actorCellId;
...
const getIsInProgress = (index: number) => inProgressPlayers.value.includes(index)
</script>
